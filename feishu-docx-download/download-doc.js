 ...
 *   --url          Feishu Wiki URL (wiki node token auto-extracted)
 *   --file-token   Drive file token (skip Wiki lookup)
...
/** Extract node_token from a Feishu Wiki URL */
function extractNodeToken(url) {
...
async function main() {
  ...
  // -------------------------------------------------------------------------
  // Step 1: Resolve Wiki URL → file_token
  // -------------------------------------------------------------------------
  if (wikiUrl) {
    const nodeToken = extractNodeToken(wikiUrl);
    if (!nodeToken) {
      console.log(JSON.stringify({ error: 'invalid_url', message: 'Cannot extract node token from URL: ' + wikiUrl }));
      process.exit(1);
    }
    ...
    if (!resolvedName) {
      resolvedName = node.title || `feishu_${resolvedToken}`;
    }
  }
  ...
  console.log(JSON.stringify({
    ...
    size_bytes: size,
    reply: `文件已下载：${resolvedName}（${formatSize(size)}），保存至：${outputPath}`,
  }));
}
