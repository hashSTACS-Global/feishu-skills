...
| 参数 | 何时需要询问 |
|---|---|
| `--summary` | 用户未明确说明日程/会议标题 |
| `--start-time` | 时间含糊（如"下午"、"明天"），询问具体时间点 |
| `--end-time` | 用户未说明结束时间或时长，询问"持续多长时间？" |
| `--attendees` | 创建会议时用户未提及参与者，询问"有哪些参与者？" |
| `--location` | 用户提到"会议室"但未指明，询问具体地点（未提则不强求） |
